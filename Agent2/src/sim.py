...
q, train_actions_history, train_returns_since_entry, percent_b_states_values, close_sma_ratio_states_value, cash_states_values = tu.trainqlearner(ticker, start_date, end_date, window = 5, gamma = 0.95, episodes = 2000, sh = 30, alp = 0.1, epsi = 0.99)
...
def qlearner(stock_table,money,inc, original_shares, commission, q_table = nq, test_data = test_df,  percent_b_states_values = percent_b_states_values, close_sma_ratio_states_value = close_sma_ratio_states_value, cash_states_values = cash_states_values, temp = temp):
    '''
    ...
    for i, val in enumerate(test_data):
        ...
        curr_port.append(port_val)
        cash_perc.append(cash_percent)
    ...
    temp['cash'] = curr_cash
    temp['cur_port'] = curr_port
    temp['cash_perc'] = cash_perc
    ...
    results = {'final_vals': final_vals, 'actions': actions, 'shares': f_shares, 'cash': f_cash, 'qtable': q_table,
               ...
               'CASH_quantiles': list(cash_states_values.values()),'markov': markov, 'actions_history' : actions_history}
    return results
...
def return_stats(stock='jpm',
    ...
    adj_list = pd.DataFrame(stock_table).reset_index().iloc[:,1:][stock_name].tolist()
