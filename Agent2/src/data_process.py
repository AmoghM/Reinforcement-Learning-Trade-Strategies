...
def create_cash_and_holdings_quantiles():
    # CASH (State 3)
    cash_list = [*range(1,11)]
    cash_list = [10*each for each in cash_list]

    cash_states_values = {}
    for i in range(len(cash_list)):
        cash_states_values[i] = cash_list[i]

    return cash_states_values
...
def get_all_states(percent_b_states_values, close_sma_ratio_states_value, cash_states_values):
    '''
